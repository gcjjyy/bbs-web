 ...
 * lrzsz algorithm: crctab[(crc >> 8) & 255] ^ (crc << 8) ^ byte
 */
...
/**
 * Calculate CRC16 for a buffer
 */
export function crc16(data: Uint8Array, initial: number = 0): number {
  let crc = initial
...
/**
 * Finalize CRC16 (add two zero bytes as per ZMODEM spec)
 */
export function crc16Finish(crc: number): number {
  crc = crc16Update(crc, 0)
  crc = crc16Update(crc, 0)
  return crc
}
