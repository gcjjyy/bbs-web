...
- `frontend/src/`: React 19 + TypeScript application code (built with Vite).
- `frontend/src/components/`: UI components such as terminal and navigation.
- `frontend/src/terminal/`: terminal emulation, Socket.IO networking, smart
  mouse, and shared terminal state (plain modules, not hooks).
- `frontend/src/hooks/`: React hooks (ZMODEM session state).
...
- `npm run install:all`: install frontend npm dependencies and server Bun
  dependencies.
- `npm run dev:frontend`: run the Vite dev server from `frontend/` (proxies
  `/socket.io` and `/api` to the local server on port 8199).
...
- `npm run verify`: frontend tests + build, then server tests + typecheck.
- `cd frontend && npm test`: run Vitest/React Testing Library tests.
- `cd server && bun run test`: run server tests.
...
Keep protocol logic in focused modules: terminal rendering belongs in
`frontend/src/terminal/`, ZMODEM code in `frontend/src/zmodem/`, and telnet/proxy
logic in `server/src/`.

## Testing Guidelines

Frontend tests use Vitest with React Testing Library. Place tests near the code
under `frontend/src/` and use the `*.test.ts(x)` naming pattern, as in
`App.test.tsx`. Server tests use `bun test` next to the code in `server/src/`.
Add tests for user-visible behavior, hook logic, and regressions in terminal or
transfer flows.
